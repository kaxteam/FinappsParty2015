...
public class RunningFragment extends Fragment {
    ...
    public static final String REST = "http://192.168.0.102:8000/api/rasp/";
    ...
    private class BackgroundData extends AsyncTask<Void, Void, PitchYawRoll> {
        ...
        protected PitchYawRoll doInBackground(Void... params) {
            ...
            while(true){
                Response<PitchYawRoll> response = webService.get(PitchYawRoll.class, REST + "isFall");
                final PitchYawRoll pyr = response.data;
                getActivity().runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if(pyr.isFall())
                            startButton.setImageResource(R.drawable.red_ball);
                        else
                            startButton.setImageResource(R.drawable.green_ball);
                    }
                });
